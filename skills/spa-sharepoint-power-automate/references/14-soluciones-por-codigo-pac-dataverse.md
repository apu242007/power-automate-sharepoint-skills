...
- **Tipo del valor.** Una expresión `@{...}` dentro de un objeto JSON devolvió texto (`"True"`). Para conservar el booleano, la expresión va sola: `"@coalesce(...)"`. Esta corrección no se probó.
...
- **Sin probar de punta a punta:** que un flow con una acción de SharePoint quede activado tras el import y escriba la fila. En la prueba, el flow con la acción quedó en borrador tras el import (causa no confirmada).
